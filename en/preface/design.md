...
Most sophisticated use cases for reuqests require to select elements relative to previous results.
A good example are supermarkets that are close to a railway station.
The supermarktes are related to the railway stations only by spatially being close to it.
...
This approach in natural language quickly ends up in muddle of relative sentences,
...
and the complex tasks are accomplished by enqueing multiple simple tasks.
This brings us to the following approach:

* Select all stations in the region of interest.
* Replace the selection by a supermarkets close to objects in the found result.
...
but the resulting two-line-solution reflects the clear separation by duties:
...
The statement ``out center`` is an output statment without further substructures.
...
that carry a speicific tag (supermarkets in one case, railway stations in the other),
the filters ``({{bbox}})`` and ``(around:100)`` perform spatial filtering.

The filter ``({{bbox}})`` matches exactly thos objects
...
The filter ``(around:100)`` gets in the given requet as input exactly the railway stations
...
This particular evaluator is an expression that is evaluated once for each selected object
...
the the expression returns an empty string.

Line 5 also contains with ``_.val`` an _evaluator_.
Its purpose is to generate the value that shall be print.
...
More important, they could be confused with actual OpenStreetMap objects and uploaded on accident.
...
e.g. railway platforms distant from railways, or addresses far away from streets.
